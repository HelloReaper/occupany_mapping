...
void OccupanyMapping(std::vector<GeneralLaserScan>& scans,std::vector<Eigen::Vector3d>& robot_poses)
{
  ...
  {
    // 获取该帧机器人位姿的栅格序号
    GridIndex robotIndex = ConvertWorld2GridIndex(p(0),p(1));

    //判断该帧机器人位姿的栅格序号，是否在自己设定的栅格地图范围内
    if(isValidGridIndex(robotIndex) == false) continue;

    //将空闲栅格的栅格序号，转化到数组序号,该数组用于存储每一个栅格的数据
    int poseindex=GridIndexToLinearIndex(robotIndex);
    pMap[poseindex]=90;
  }
...
int main(int argc, char** argv)
{
  ...
  std::string basePath = "/home/lk/Codes/slam_ws/src/occupany_mapping/test_data_4";
